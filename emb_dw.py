...
def random_walk(G, start_node, path_length):
    walk = [start_node]
    while len(walk) < path_length:
        cur = walk[-1]
        cur_neighbors = G[cur]
        if len(cur_neighbors) > 0:
            next_node = random.choice(cur_neighbors)[0]
...
for start_node in range(len(G)):
    for i in range(num_walks_per_node):
        walk = random_walk(G, start_node, path_length)
        all_walks.append(walk)

patient_patient_path = [[(walk[i], -1) for i in range(len(walk))] for walk in all_walks]
...
with open('./PubMed/train_corpus.txt', 'w') as file:
    for paragrapth in train_text:
        file.write(paragrapth + '\n')

with open('./PubMed/val_corpus.txt', 'w') as file:
    for paragrapth in val_text:
        file.write(paragrapth + '\n')

datasets = load_dataset('text', data_files={'train': './PubMed/train_corpus.txt', 'validation': './PubMed/val_corpus.txt'})
